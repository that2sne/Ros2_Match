...
using ebase::fusion::matcher::PerceptDataT;
...
bool PerceptMatching::Update(MatchInfo & mi, const PerceptInfo & pi)
{
  ...
  //printf("Update:[%d] cam[%d], obj_id[%d]\n", mi.fusion_id, mi.cam_id, pi.obj_id); fflush(stdout);
  if (mi.cam_id == pi.obj_id) {
    float match_box_area = (float)(mi.bbox.width * mi.bbox.height);
    float percept_box_area = (float)(pi.width * pi.height);
    //printf("match_box_area %f percept_box_area %f\n", match_box_area , percept_box_area); fflush(stdout);
